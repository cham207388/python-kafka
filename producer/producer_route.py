...
from producer.utils import generate_fake_student_obj, num_of_partitions


class ProducerController:
    def __init__(
        self, producer_service: ProducerService, student_service: StudentService
    ):
        self.producer_service = producer_service
        self.student_service = student_service
        self.router = APIRouter(prefix="/api/v1", tags=["Health Check"])
        self.router.post(path="/students", status_code=201, response_model=Student)(
            self.produce_student
        )
    ...
    def produce_student(self):
        ...
        key = student.id
        partition = self.producer_service.get_partition(
            key, num_partitions=num_of_partitions
        )
